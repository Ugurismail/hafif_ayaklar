...
LOGIC_LEVEL_TEST_CONFIG = {
    ...
    "focus_points": [
        ...
        "Aksiyom, teorem, türetilmiş kural ve metateoremi farklı katmanlar olarak görmek.",
    ],
    "modules": [
        {
            "id": "foundations",
        ...
        },
        {
            ...
            "description": "Niceleyiciler, kapsam, kimlik, fonksiyon sembolleri ve doğal dilden sembole geçiş.",
            "sample_size": 10,
        },
        {
            "id": "axiomatic",
            "title": "Aksiyomatik Sistem ve Metateori",
            "description": "Aksiyom şemaları, teorem üretimi, türetilmiş kurallar, güvenirlik, tamlık ve sınırlar.",
            "sample_size": 10,
        },
    ],
}
...
LOGIC_LEVEL_TEST_BANK = [
    ...
    _question("logic-foundations-004", "foundations", "'Yalnızca çalışırsan geçersin' ifadesi mantıksal olarak en iyi nasıl okunur?", ["Geçmek çalışmak için yeterlidir", "Çalışmak geçmek için zorunludur", "Çalışmak geçmek için yeterlidir", "Geçmek ile çalışmak eşdeğerdir"], "Çalışmak geçmek için zorunludur", "'Yalnızca' ifadesi zorunlu koşul kurar; geçmek varsa çalışma da vardır."),
    ...
    _question("logic-propositional-005", "propositional", "Ayrık tasım hangi kalıptır?", ["p∨q, ¬p ⟹ q", "p→q, ¬q ⟹ ¬p", "p∧q ⟹ p", "p↔q ⟹ p∨q"], "p∨q, ¬p ⟹ q", "Ayrıklardan biri elenince öteki kalır."),
    _question("logic-propositional-006", "propositional", "'p yalnızca q ise' nasıl okunur?", ["p → q", "q → p", "p ∧ q", "p ↔ q"], "p → q", "'Yalnızca' yönü sonuç tarafına q'yu yerleştirir."),
    _question("logic-propositional-007", "propositional", "Totoloji için en iyi tanım hangisidir?", ["Her satırda yanlış olan formül", "En az bir satırda doğru olan formül", "Her satırda doğru olan formül", "Yalnız iki satırlı formül"], "Her satırda doğru olan formül", "Totoloji bütün yorumlarda doğrudur."),
    _question("logic-propositional-008", "propositional", "Çelişki nedir?", ["Her yorumda doğru formül", "Bazı yorumlarda doğru formül", "Her yorumda yanlış formül", "Koşullu her formül"], "Her yorumda yanlış formül", "Çelişki hiçbir yorumda doğrulanmaz."),
    ...
    _question("logic-predicate-002", "predicate", "'Bazı öğrenciler çalışkandır' için en uygun çeviri hangisidir?", ["∀x(Sx → Cx)", "∃x(Sx ∧ Cx)", "∃x(Sx → Cx)", "∀x(Sx ∧ Cx)"], "∃x(Sx ∧ Cx)", "Varlık ve özellik varoluşsal birleşim ile kurulur."),
    ...
    _question("logic-predicate-006", "predicate", "Kimlik için temel simge hangisidir?", ["≡", "=", "⊨", "↔"], "=", "Kimlik/eşitlik biçimsel dilde '=' ile gösterilir."),
    ...
    _question("logic-predicate-008", "predicate", "Fonksiyon sembolü neyi temsil eder?", ["Doğruluk değerini", "Bir terim üreten işlemi", "Yalnız niceleyiciyi", "Bir teorem listesini"], "Bir terim üreten işlemi", "Fonksiyon sembolü doğruluk değeri üretmez; nesneye gönderimde bulunan bir terim kurar."),
    _question("logic-predicate-009", "predicate", "Önek normal biçimde ne olur?", ["Bütün niceleyiciler başa toplanır", "Bütün bağlaçlar silinir", "Kimlik yasaklanır", "Tüm terimler eşitlenir"], "Bütün niceleyiciler başa toplanır", "Önek dönüşümü niceleyici bloğunu, serbest ve bağlı değişkenleri koruyarak formülün önüne taşır."),
    ...
    _question("logic-predicate-011", "predicate", "'Yalnız bir kral vardır' gibi ifade hangi sorunu açar?", ["Belirli betimleme ve tekillik yükü", "Modus tollens", "Doğruluk ağacının kapanması", "Çift olumsuzlama"], "Belirli betimleme ve tekillik yükü", "Belirli betimlemeler varlık ve tekillik yükünü birlikte taşır."),
    _question("logic-predicate-012", "predicate", "Tümel örnekleme kabaca ne yapar?", ["Bir örnekten tümel üretir", "Tümel önermeden tekil örnek çıkarır", "Kimliği siler", "Ayrık bağlaç kurar"], "Tümel önermeden tekil örnek çıkarır", "∀ ile başlayan ifade uygun terim için örneklenir."),

    _question("logic-axiomatic-001", "axiomatic", "Aksiyom ile teorem arasındaki temel fark nedir?", ["Aksiyom başlangıç kabulü, teorem türetilmiş formüldür", "Teorem daha kısadır", "Aksiyom daha doğrudur", "Aralarında fark yoktur"], "Aksiyom başlangıç kabulü, teorem türetilmiş formüldür", "Aksiyomlar sistemin başlangıç noktalarıdır; teoremler sistem içinde kanıtlanmış formüllerdir."),
    _question("logic-axiomatic-002", "axiomatic", "Türetilmiş kural neyi ifade eder?", ["Keyfî kestirme", "Temel kurallardan kanıtlanabildiği için kısaltma olarak kullanılan kural", "Yeni aksiyom", "Yalnız doğal dil kuralı"], "Temel kurallardan kanıtlanabildiği için kısaltma olarak kullanılan kural", "Türetilmiş kuralın meşruiyeti, temel kurallarla kanıtlanabilir olmasından gelir."),
    _question("logic-axiomatic-003", "axiomatic", "Güvenirlik neyi garanti eder?", ["Sistem her doğruyu bulur", "Sistem semantik olarak geçersiz bir sonucu türetmez", "Bütün ispatlar kısadır", "Her teorem aksiyomdur"], "Sistem semantik olarak geçersiz bir sonucu türetmez", "Güvenirlik yönü şudur: Γ ⊢ φ ise Γ ⊨ φ."),
    _question("logic-axiomatic-004", "axiomatic", "Tamlık neyi söyler?", ["Semantik sonuç sentaktik olarak da türetilebilir", "Sistem daima hızlı çalışır", "Sistem yalnız doğruluk ağacı kullanır", "Her aksiyom bağımsızdır"], "Semantik sonuç sentaktik olarak da türetilebilir", "Tamlık yönü şudur: Γ ⊨ φ ise Γ ⊢ φ."),
    _question("logic-axiomatic-005", "axiomatic", "Kompaktlık hangi fikri taşır?", ["Bütün formüller kısa olur", "Doyurulamazlık sonlu bir altkümeyle de gösterilebilir", "Her teorem aksiyomdur", "'Ancak' yönü tersine döner"], "Doyurulamazlık sonlu bir altkümeyle de gösterilebilir", "Kompaktlık, sonsuz küme davranışını sonlu tanıklık fikrine bağlar."),
    _question("logic-axiomatic-006", "axiomatic", "Bağımsızlık neyi tartışır?", ["Bir aksiyomun ötekilerden türetilememesini", "Her aksiyomun teorem olmasını", "Tüm sistemlerin tamamlanamazlığını", "Doğruluk tablosu sayısını"], "Bir aksiyomun ötekilerden türetilememesini", "Bağımsızlık, aksiyom kümesinin iç yapısına dair bir metateorik özelliktir."),
    _question("logic-axiomatic-007", "axiomatic", "Aksiyomatik sistemde modus ponens'in yeri nedir?", ["Sıradan örnek", "Merkezi temel çıkarım mekanizması", "Yalnız türetilmiş kural", "Niceleyici eşleniği"], "Merkezi temel çıkarım mekanizması", "Hilbert tarzı sistemler çoğu yükü aksiyom şemaları ve modus ponens üstüne kurar."),
    _question("logic-axiomatic-008", "axiomatic", "'p→p' neden pedagojik olarak önemlidir?", ["Çünkü süslüdür", "Basit bir teoremin nasıl üretildiğini gösterir", "Çünkü yüklem mantığı formülüdür", "Çünkü asla kanıtlanamaz"], "Basit bir teoremin nasıl üretildiğini gösterir", "Kısa teoremler sistemin üretim mantığını görünür kılar."),
    ...
    _question("logic-axiomatic-010", "axiomatic", "Doğal türetim ile aksiyomatik sistem arasındaki farkın iyi özeti hangisidir?", ["Biri sentaktik, öteki anlamsaldır", "Biri kural adımı merkezli, öteki aksiyom şeması ve modus ponens merkezlidir", "Aralarında fark yoktur", "Biri yalnız yüklem mantığı içindir"], "Biri kural adımı merkezli, öteki aksiyom şeması ve modus ponens merkezlidir", "İkisi de sentaktik biçimsel sistemdir; kanıt mimarileri farklıdır."),
    _question("logic-axiomatic-011", "axiomatic", "Teorem kütüphanesi neden önemlidir?", ["Yalnız tarih tutmak için", "Tekrar kullanılan kanıt kalıplarını ekonomik biçimde çağırmak için", "Aksiyomları iptal etmek için", "Semantiği yok etmek için"], "Tekrar kullanılan kanıt kalıplarını ekonomik biçimde çağırmak için", "İleri çalışma, teorem ve lemma kütüphanesi üstüne kurulur."),
    _question("logic-axiomatic-012", "axiomatic", "Karar verilebilirlik neyi sorar?", ["Bir karar prosedürü var mı?", "Formül güzel mi?", "Aksiyom bağımsız mı?", "Kimlik sembolü var mı?"], "Bir karar prosedürü var mı?", "Karar verilebilirlik, mekanik bir karar yönteminin bulunup bulunmadığına dair metateorik sorudur."),
]
...
def build_logic_level_test(seed=None):
    rng = Random(str(seed)) if seed is not None else SystemRandom()
    ...
    return {
        "attempt_id": str(seed) if seed is not None else "temporary",
