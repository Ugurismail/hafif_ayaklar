...
E34_SIGNATURE = {
    ...
    "predicates": {
        ...
        "K": {
            "arity": 1,
            "reading": "x koordinatör",
            "roles": ["koordinatör olan"],
        },
        "T": {
            "arity": 2,
            "reading": "x, y'yi tanıyor",
...
def _candidate_e34():
    lesson = _lesson(
        ...
        "Belirsizlik ve Sembolleştirme Atölyesi",
        "Gerçek doğal dil cümlelerini alan, anahtar, varsayım, ara yeniden ifade, kapsam dalları, nihai formül, geri çeviri ve anlam kaybı raporuyla savunulabilir FOL çözüm dosyalarına dönüştürür.",
        "Uçtan uca FOL sembolleştirme ve aşama çıkış projesi",
        55,
        [
            "ders-fol-resmi-sozdizim-baglanma-yerine-koyma",
        ...
        [
            "fol.translation_plan",
            "fol.ambiguity_branch",
            "fol.back_translate",
            "fol.loss_report",
            "fol.stage_project",
        ],
        [
            "Sembolleştirmeden önce alanı, adları, yüklem aritelerini ve bağlam varsayımlarını yazmak.",
            "Karmaşık cümleyi kontrollü doğal dil ara basamaklarına ayırmak.",
            "Niceleyici sırası, olumsuzluk ve yalnız/hariç kaynaklı belirsizlikleri ayrı okumalara dallandırmak.",
            "Her nihai formülü sözdizim, bağlanma, kapsam ve bağıntı yönü bakımından denetlemek.",
            "Formülü doğal dile geri okuyarak hedef cümleyle karşılaştırmak.",
            "FOL'nin dışarıda bıraktığı vurgu, zaman, nedensellik, bulanıklık ve bağlamsal imaları raporlamak.",
            "Başka bir çözümdeki ilk kategori, arite, yön, kapsam veya bağlanma hatasını bulup onarmak.",
        ],
        [
            ("Çözüm dosyası", "Kaynak cümleden kayıp raporuna kadar bütün sembolleştirme kararlarını görünür tutan kayıt."),
            ("Alan kararı", "Niceleyicilerin hangi nesneler üzerinde dolaştığını açıkça belirleme."),
            ("Kontrollü yeniden ifade", "Yüzey cümlesini sembolleştirmeden önce kapsam ve bağıntı rollerini açık doğal dille yeniden yazma."),
            ("Okuma dalı", "Aynı yüzey cümlesinin farklı bağlamlarda savunulabilen ayrı mantıksal yapısı."),
            ("Bağlam koşulu", "Belirli bir okumanın seçilmesini gerekçelendiren durum veya konuşmacı niyeti."),
            ("Geri çeviri", "Nihai formülü sembol anahtarına göre yeniden doğal dilde okuma."),
            ("Kayıp raporu", "FOL soyutlamasının temsil etmediği anlam boyutlarını açıkça listeleme."),
            ("İlk hata", "Bir çözümde sonraki adımları ilk kez bozan en erken kategori, anahtar, yön, kapsam veya bağlanma kararı."),
        ],
        [
            _section(
                "Önce alanı ve bağlam varsayımlarını sabitle",
                "Niceleyici anlamı alanla değişir. 'Herkes' yalnız atölyedeki insanlar üzerinde dolaşıyorsa bu karar formülden önce yazılmalıdır; varlık ve tekillik varsayımları da ayrıca kaydedilir.",
                "Her yeni metin veya bağlam sembolleştirilmeye başlanırken.",
                "kaynak → alan → bağlam varsayımları",
                "Niceleyici sözcüklerini işaretle; hangi nesnelerin aday olduğunu ve cümlenin varlık önkabullerini açık listele.",
                "Alanı formülden tahmin ettirmek veya çalışma boyunca sessizce değiştirmek.",
                [("Alan: atölyedeki insanlar", "∀ ve ∃ yalnız bu kişiler üzerinde dolaşır."), ("Varsayım: Ada alanda", "a adı bir alan nesnesini gösterir."), ("Varlık varsayımı yok", "Tümel koşul boş alt sınıfta da doğru olabilir.")],
                ("Alan kararını çözümün ilk satırına yazmak.", "'Herkes' sözcüğünü evrensel bütün varlıklar diye okumak.", "Formül ancak ilan edilmiş alanla birlikte bir çeviridir."),
            ),
            _section(
                "Sembol anahtarını arite ve rol sırasıyla kur",
                "T(x,y) yalnız 'x, y'yi tanıyor' diye sabitlenirse T(a,b) ile T(b,a) güvenle ayrılır. Yüklem okuması örnekten örneğe değiştirilemez.",
                "Metindeki özellik ve bağıntılar belirlendikten sonra.",
                "F: x mentor · T: x, y'yi tanıyor",
                "Her yükleme tek okuma, arite ve sıralı rol listesi ver; aynı kavram için gereksiz ikinci sembol açma.",
                "T'yi bir örnekte tanıyor, başka örnekte tanınıyor yönünde kullanmak.",
                [("F(x)", "x mentor."), ("T(x,y)", "x tanıyan, y tanınan."), ("D(x,y)", "x danışan, y danışılan.")],
                ("Her bağıntı için rol tablosu yazmak.", "Doğal dil sözcük sırasını argüman sırası sanmak.", "Arite ve rol sırası anahtar sözleşmesidir."),
            ),
            _section(
                "Kontrollü doğal dil ara basamağı kur",
                "'Her mentor bir katılımcıyı tanıyor' önce 'Her x için: x mentorsa, x'in tanıdığı en az bir y vardır ve y katılımcıdır' diye açılır. Ara basamak karma doğal dil/FOL olabilir; nihai cevap olamaz.",
                "Cümlede kısıtlama, bağıntı veya birden fazla nicelik bulunduğunda.",
                "yüzey cümlesi → rol ve kapsam açık yeniden ifade → FOL",
                "Ana niceleyiciyi, kısıtlayıcı sınıfı, tanık bağımlılığını ve bağıntı yönünü doğal dilde ayrı satırlara çıkar.",
                "Sözcükleri soldan sağa sembollerle değiştirip yapı kurulduğunu sanmak.",
                [("Her mentor", "∀x(F(x)→...)."), ("bir katılımcı", "mentor x'e bağlı ∃yG(y)."), ("onu tanıyor", "T(x,y).")],
                ("Her ara satırı geri okunabilir tutmak.", "Ara karışımı nihai formül diye bırakmak.", "Ara dil düşünme iskelesidir; sonuç tam FOL cümlesidir."),
            ),
            _section(
                "Gerçek belirsizliği ayrı okuma dallarına ayır",
                "'Her katılımcı bir mentoru tanıyor' ayrı mentorlar veya bağlama göre ortak bir mentor okuması taşıyabilir. Bu iki yapı tek belirsiz formülde saklanmaz.",
                "Yüzey cümlesi niceleyici sırasını, olumsuzluk kapsamını veya yalnızlık önkabulünü sabitlemediğinde.",
                "dal A: ∀x∃y... · dal B: ∃y∀x...",
                "Her okuma için kontrollü yeniden ifade, bağlam koşulu, ayrı formül ve geri çeviri yaz.",
                "Bağlam belirtmeden bir okuma dalını tek resmi doğru ilan etmek.",
                [("Her katılımcının ayrı mentoru olabilir", "∀x(G(x)→∃y(F(y)∧T(x,y)))."), ("Tek ortak mentor kastediliyor", "∃y(F(y)∧∀x(G(x)→T(x,y)))."), ("Herkes geç kalmadı", "¬∀xH(x) veya ∀x¬H(x).")],
                ("Okumaların ayrıldığı ilk yapısal kararı yazmak.", "Belirsizliği parantezsiz veya serbest değişkenli formülle temsil etmek.", "Her dal kendi başına iyi kurulmuş bir cümledir."),
            ),
            _section(
                "Nihai formülü dört geçişte denetle",
                "Bir formül önce kategori ve arite, sonra bağlanma, sonra kapsam/niceleyici sırası, son olarak bağıntı yönü bakımından kontrol edilir. İlk bozuk katman düzeltilmeden sonraki yoruma geçilmez.",
                "Her nihai formül ve akran çözümü incelenirken.",
                "1 sözdizim · 2 bağlanma · 3 kapsam · 4 yön",
                "Bağımsız ayrıştırıcıyı çalıştır; serbest değişkenleri listele; niceleyici ağacını ve argüman rollerini geri oku.",
                "Formül hedefe benziyor diye yalnız sonuca bakmak veya aynı anda bütün hataları sıralamak.",
                [("R(a)", "İlk hata aritedir."), ("∀xT(x,y)", "İlk yapısal sorun serbest y'dir."), ("∃y∀xT(x,y)", "Sözdizim doğru; hedef ∀x∃y ise sıra hatasıdır.")],
                ("İlk hata kodunu ve konumunu kaydetmek.", "Bir kapsam hatasını sözcük seçimiyle açıklamak.", "Katmanlı denetim düzeltmeyi eyleme dönüştürür."),
            ),
            _section(
                "Geri çeviriyle yapısal kararları sınamak",
                "∀x∃yT(x,y) 'herkes birini tanıyor'; ∃y∀xT(x,y) 'herkesin tanıdığı biri var' diye geri okunur. Geri çeviri hedefle uyuşmuyorsa formül düzeltilir.",
                "Sözdizim denetimi geçen her formülde.",
                "FOL → anahtar okuması → kontrollü doğal dil → kaynak karşılaştırması",
                "Formülü dıştan içe oku; her niceleyicide tanığın önceki seçime bağlı mı ortak mı olduğunu ve her bağıntıda rol sırasını söyle.",
                "Formülü yalnız sembol adlarıyla okumak veya doğal dilde eksik niceliği örtmek.",
                [("∀x∃yT(x,y)", "Her kişi için tanıdığı biri vardır."), ("∃y∀xT(x,y)", "Bir kişi vardır; herkes onu tanır."), ("∀x∃yT(y,x)", "Herkes birisi tarafından tanınır.")],
                ("Geri çeviriyi kaynak cümleden bağımsız üretmek.", "Beklenen cümleyi formüle bakmadan tekrar etmek.", "Geri çeviri bağımsız bir hata denetimidir."),
            ),
            _section(
                "Kayıp anlamı dürüstçe raporla",
                "FOL seçilen alandaki niceleme, kimlik ve bağıntı yapısını gösterir; zaman, kiplik, vurgu, nedensellik, duygusal ton, yaklaşık nicelik ve konuşma imasını çoğu kez temsil etmez.",
                "Çözüm tamamlandıktan ve formül hedef yapıyı yakaladıktan sonra.",
                "korunan yapı + dışarıda kalan anlam + ek varsayım",
                "Kaynak cümledeki her anlam boyutunu tarayıp formülde açık karşılığı olmayanları kayıp veya bilinçli soyutlama olarak listele.",
                "Doğru FOL formülünün doğal dildeki bütün anlamı eksiksiz koruduğunu iddia etmek.",
                [("'nihayet'", "Zamanlama ve tutum vurgusu kaybolur."), ("'yaklaşık herkes'", "Klasik ∀ ile bulanıklık kaybolur."), ("'çünkü'", "Salt FOL bağıntısı nedensellik anlamını kendiliğinden vermez.")],
                ("Kayıp ile yanlış çeviriyi ayırmak.", "Temsil edilmeyen her ayrıntıyı formül hatası saymak.", "Biçimselleştirme amaçlı ve belgelenmiş bir soyutlamadır."),
            ),
            _section(
                "Akran incelemesinde ilk bozuk kararı onar",
                "İyi geri bildirim yalnız doğru formülü vermez. 'y serbest kaldı; önce tanık niceleyicisini ekle' gibi ilk hata sınıfını, etkisini ve bir sonraki kontrolü belirtir.",
                "Başka bir öğrencinin çözüm dosyası değerlendirilirken.",
                "bulgu → etki → en küçük onarım → yeniden denetim",
                "Dokuz parçalı dosyayı sırayla tara; ilk tutarsız kararda dur; neden sonraki adımları bozduğunu açıkla ve en küçük düzeltmeyi öner.",
                "Yalnız yanlış/doğru etiketi vermek veya bütün çözümü öğrencinin yerine yeniden yazmak.",
                [("Alan belirtilmemiş", "Niceleyici okuması belirsizdir."), ("T rolleri ters", "Bütün örneklerde yön bozulur."), ("y serbest", "Nihai ifade cümle değildir.")],
                ("Düzeltmeden sonra aynı dört geçişi yeniden çalıştırmak.", "İlk hata onarılmadan stil ayrıntılarına geçmek.", "Akran incelemesi izlenebilir bir hata ayıklamadır."),
            ),
        ],
        [
            _worked("Alan: atölyedeki insanlar", "Niceleyicilerin dolaştığı nesneler formülden önce sabitlenir.", "1 · Alan"),
            _worked("T(x,y): x, y'yi tanıyor", "Arite ve iki sıralı rol anahtarda değişmez.", "2 · Anahtar"),
            _worked("Her mentor x için ona bağlı bir katılımcı y", "Yüzey cümlesi kontrollü ara dile açılır.", "3 · Yeniden ifade"),
            _worked("∀x(F(x) → ∃y(G(y) ∧ T(x,y)))", "Her mentor için ayrı katılımcı tanığına izin verir.", "4 · Nihai formül"),
            _worked("∃y(G(y) ∧ ∀x(F(x) → T(x,y)))", "Aynı katılımcı bütün mentorlar için ortaklaştırılır.", "Kapsam dalı"),
            _worked("¬∀xH(x)", "Hepsi geç kalmadı; en az bir karşı örnek vardır.", "Olumsuzluk dalı A"),
            _worked("∀x¬H(x)", "Hiç kimse geç kalmadı.", "Olumsuzluk dalı B"),
            _worked("(K(a) ∧ ∀x(K(x) → x=a))", "Ada koordinatördür ve başka koordinatör yoktur.", "Yalnızlık"),
            _worked("∀x∃y(x≠y ∧ T(x,y))", "Başka kişi koşulu kimlikle açık kurulur.", "Kimlik"),
            _worked("Geri: Her mentorun tanıdığı en az bir katılımcı vardır", "Formül bağımsız doğal dile çevrilerek hedefle karşılaştırılır.", "Geri çeviri"),
            _worked("Kayıp: zaman, vurgu ve gerekçe", "FOL'nin bilinçli olarak dışarıda bıraktığı anlam yazılır.", "Kayıp raporu"),
            _worked("∀x(F(x) → ∃y(G(y) ∧ T(y,x)))", "Arite doğru olsa da mentor tanınan role geçirilmiştir.", "Yön hatası", "bad"),
        ],
        [
            "Alanı ve bağlam varsayımlarını yazmadan niceleyici formülüne başlamak.",
            "Yüklem aritesini veya bağıntı rol sırasını örnekler arasında değiştirmek.",
            "Sözcük sırasını doğrudan niceleyici sırası sanmak.",
            "Kontrollü ara dili nihai FOL formülü diye bırakmak.",
            "Belirsiz cümleye bağlam vermeden tek resmi cevap dayatmak.",
            "Hepsi değil ile hiçbiri okumalarını birleştirmek.",
            "Yalnız/hariç cümlesindeki varlık önkabulünü açıklamamak.",
            "Nihai formülde serbest değişken bırakmak.",
            "Bağıntı yönünü geri çeviride sessizce düzeltmek.",
            "Mantıksal eşdeğerliği bütün doğal dil anlamının eşitliği sanmak.",
            "Kayıp raporunu formül hataları listesiyle karıştırmak.",
            "Akran çözümünde ilk hatayı bulmadan sonuç formülünü dayatmak.",
        ],
        _practice(
            [
                ("Sembolleştirme dosyasının ilk kararı hangisidir?", ["Alan ve bağlam", "Nihai formül", "Doğruluk tablosu", "Kanıt kuralı"], "Alan ve bağlam", "Niceleyiciler ancak alanla yorumlanır.", "Temel"),
                ("Bağıntı anahtarında arite yanında ne yazılmalıdır?", ["Sıralı roller", "Doğruluk değeri", "Sayfa numarası", "Kanıt satırı"], "Sıralı roller", "Argüman yönü anahtar sözleşmesidir.", "Temel"),
                ("Kontrollü yeniden ifadenin amacı nedir?", ["Kapsam ve rolleri görünür kılmak", "Nihai formül yerine geçmek", "Alanı kaldırmak", "Eşdeğerlik kanıtlamak"], "Kapsam ve rolleri görünür kılmak", "Ara dil yapısal kararları açar.", "Temel"),
                ("∀x∃yT(x,y) ile ∃y∀xT(x,y) nerede ayrılır?", ["Tanığın bağımlı mı ortak mı olduğunda", "T'nin aritesinde", "Değişken adının yazımında", "Alan sembolünde"], "Tanığın bağımlı mı ortak mı olduğunda", "Niceleyici sırası tanık bağımlılığını değiştirir.", "Orta"),
                ("Belirsiz cümle için doğru yöntem nedir?", ["Her okumayı bağlam koşuluyla ayrı formüle yazmak", "Parantezleri kaldırmak", "Tek formül seçmek", "Serbest değişken bırakmak"], "Her okumayı bağlam koşuluyla ayrı formüle yazmak", "Belirsizlik dallandırılır.", "Orta"),
                ("Nihai formülün ilk otomatik denetimi nedir?", ["Sözdizim ve arite", "Estetik", "Tarih", "Geçerlilik kanıtı"], "Sözdizim ve arite", "Bozuk yapı üzerinde semantik yorum yapılmaz.", "Orta"),
                ("Geri çeviri neden kaynak cümleden bağımsız üretilir?", ["Formülün gerçekten ne dediğini sınamak için", "Metni uzatmak için", "Alanı değiştirmek için", "Kanıt kurmak için"], "Formülün gerçekten ne dediğini sınamak için", "Beklenen cümleyi tekrarlamak hata yakalamaz.", "Orta"),
                ("∀x∃yT(y,x) nasıl geri okunur?", ["Herkes birisi tarafından tanınır", "Herkes birini tanır", "Birini herkes tanır", "Kimse kimseyi tanımaz"], "Herkes birisi tarafından tanınır", "y tanıyan, x tanınandır.", "Orta"),
                ("Kayıp raporuna hangisi girer?", ["FOL'de temsil edilmeyen zaman ve vurgu", "Eksik parantez", "Yanlış arite", "Serbest değişken"], "FOL'de temsil edilmeyen zaman ve vurgu", "Diğerleri çeviri/sözdizim hatasıdır.", "İleri"),
                ("Mantıksal eşdeğer iki formül neyi garanti etmez?", ["Doğal dilde bütün vurgu ve imaların aynı olmasını", "Aynı doğruluk koşullarını", "Formül olmalarını", "Sembol içermelerini"], "Doğal dilde bütün vurgu ve imaların aynı olmasını", "Biçimsel soyutlama pragmatik ayrıntıları dışarıda bırakabilir.", "İleri"),
                ("Akran incelemesinde en yararlı ilk geri bildirim nedir?", ["İlk hata, etkisi ve küçük onarım", "Yalnız yanlış etiketi", "Bütün doğru cevabı vermek", "Stil yorumu"], "İlk hata, etkisi ve küçük onarım", "İzlenebilir onarım öğrenmeyi destekler.", "İleri"),
                ("Faz E çıkış dosyasında nihai formüller neyi geçmelidir?", ["Sözdizim, bağlanma, kapsam ve yön denetimini", "FOL kanıt kurallarını", "Model geçerlilik ispatını", "Modal mantık testini"], "Sözdizim, bağlanma, kapsam ve yön denetimini", "Semantik ve kanıt Faz F'de başlar.", "İleri"),
            ]
        ),
        {
            "prompt": "Kısa bir gerçek doğal dil paragrafından dokuz parçalı FOL çözüm dosyası üret: kaynak, alan, anahtar, varsayımlar, kontrollü yeniden ifade, okuma dalları, nihai formüller, geri çeviri ve kayıp raporu.",
            "starter": "Henüz formül yazma; önce metindeki nicelik, olumsuzluk, yalnızlık ve bağıntı sözcüklerini farklı renklerle işaretle.",
            "checks": [
                "Kaynak cümle değiştirilmeden kaydedildi",
                "Alan ve bağlam varsayımları açıklandı",
                "Her yüklemin aritesi ve rol sırası sabitlendi",
                "Kontrollü yeniden ifade bütün nicelik ilişkilerini açtı",
                "Gerçek belirsizlikler bağlam koşullarıyla dallandırıldı",
                "Nihai ifadelerin tamamı kapalı ve iyi kurulmuş formüller oldu",
                "Her formül kapsam ve yön korunarak geri çevrildi",
                "FOL'nin dışarıda bıraktığı anlam kayıp raporuna yazıldı",
                "Akran incelemesi ilk hatayı ve en küçük onarımı gösterdi",
            ],
            "solution": "Tek bir örnek cevap yoktur; kabul için her okumanın bağlamı, iyi kurulmuş formülü ve bağımsız geri çevirisi birbiriyle tutarlı olmalıdır.",
        },
        [
            _production_task(
                "Kendi alanından 4-6 cümlelik bir paragraf seç ve Faz E çıkış portföyünü oluştur; en az bir bağıntı yönü, bir çoklu niceleyici, bir olumsuzluk kapsamı, bir kimlik/sayı ve bir gerçek belirsizlik içersin.",
                [
                    "Dokuz parçalı çözüm dosyasının tamamı bulunuyor.",
                    "Alan, adlar, ariteler ve rol sıraları değişmeden kullanılıyor.",
                    "En az bir ∀∃/∃∀ bağımlılık karşılaştırması var.",
                    "En az bir hepsi değil/hiçbiri ayrımı var.",
                    "En az bir kimlik, farklılık veya tam sayı koşulu var.",
                    "Belirsiz okuma dalları ayrı bağlam koşulları taşıyor.",
                    "Bütün nihai formüller sözdizim denetiminden geçiyor ve kapalı.",
                    "Geri çeviriler formülden bağımsız üretilmiş.",
                    "Kayıp raporu vurgu, zaman, nedensellik veya bulanıklıktan en az ikisini değerlendiriyor.",
                    "Akran incelemesi ilk hatayı, etkisini ve onarımını kaydediyor.",
                ],
                "Portföy tek bir 'doğru formül' aramaz; açık varsayım, savunulabilir okuma, sözdizimsel doğruluk ve dürüst soyutlama raporunu birlikte değerlendirir.",
                "Metin kaynağı",
                ["Akademik paragraf", "Gündelik konuşma", "Kısa haber", "Kural metni", "Kendi yazdığın paragraf"],
                "Kişisel veri içermeyen, bağlamı açıklanabilir kısa bir metin seç.",
            ),
        ],
        [
            "Dokuz parçalı yeni bir FOL çözüm dosyası üretme.",
            "Alan, anahtar ve bağlam varsayımlarını formülden önce sabitleme.",
            "Gerçek kapsam belirsizliğinde iki savunulabilir form ve bağlam gerekçesi verme.",
            "Bütün nihai formülleri kapalı ve sözdizimsel olarak geçerli kurma.",
            "Her formülü bağımsız geri çeviriyle yön ve kapsam bakımından sınama.",
            "FOL soyutlamasının kaybettiği anlamı açık raporlama.",
            "Akran çözümündeki ilk yapısal hatayı bulup açıklayıcı onarım önerme.",
        ],
        [
            "Alan kararı bir niceleyici formülünün anlamını nasıl değiştirir?",
            "Kontrollü yeniden ifade ile nihai formülün görevleri neden ayrıdır?",
            "Bir kapsam belirsizliği hangi koşulda iki ayrı okuma dalı gerektirir?",
            "Geri çeviri bağıntı yönü hatasını nasıl görünür kılar?",
            "Kayıp anlam ile yanlış sembolleştirme nasıl ayrılır?",
            "Akran incelemesinde neden ilk hatadan başlanır?",
        ],
        "F35'te artık dili kurmayı bırakıp yorum ve model kavramlarına geçecek; alan nesneleri, gönderim işlevi ve yüklem uzantılarıyla FOL cümlelerinin bir modelde nasıl doğru olduğunu tanımlayacağız.",
        ["forallx-fol-ambiguity", "forallx-multiple-generality", "forallx-fol-sentences", "mit-logic-sequence"],
        "Bu atölye FOL çevirisini tek cevaplı kalıp eşleştirmeden ayırır. Kabul edilen her okuma bağlam koşulu, kapalı formül, geri çeviri ve kayıp raporu taşır; model doğruluğu ve kanıt Faz F'ye ertelenir.",
        ["ders-31-dogal-dilden-yuklem-mantigina-ii"],
    )
    lesson["fol_signature"] = E34_SIGNATURE
    lesson["syntax_scope"] = {
        "introduced": [
            "translation_workflow",
            "ambiguity_branching",
            "back_translation_audit",
            "meaning_loss_report",
            "peer_first_error_review",
        ],
        "review_only": [
            "quantifier_dependency",
            "quantifier_negation",
        ...
        "locked_until_later": [
            ...
            "fol_proof_rules",
            "metatheoretic_equivalence_proof",
        ],
    }
    lesson["syntax_fixtures"] = [
        _syntax_fixture("e34-mentor-dependent", "∀x(F(x) → ∃y(G(y) ∧ T(x,y)))", accepted=True, category="sentence", explanation="Her mentor için ayrı tanık seçilebilir."),
        _syntax_fixture("e34-mentor-shared", "∃y(G(y) ∧ ∀x(F(x) → T(x,y)))", accepted=True, category="sentence", explanation="Katılımcı bütün mentorlar için ortaktır."),
        _syntax_fixture("e34-not-all-late", "¬∀xH(x)", accepted=True, category="sentence", explanation="Tümel iddia geniş kapsamda yadsınır."),
        _syntax_fixture("e34-none-late", "∀x¬H(x)", accepted=True, category="sentence", explanation="Her kişi için geç kalma yadsınır."),
        _syntax_fixture("e34-only-ada", "(K(a) ∧ ∀x(K(x) → x=a))", accepted=True, category="sentence", explanation="Ada'nın koordinatörlüğü ve teklik birlikte kurulur."),
        _syntax_fixture("e34-other", "∀x∃y(x≠y ∧ T(x,y))", accepted=True, category="sentence", explanation="Başka kişi koşulu kapalıdır."),
        _syntax_fixture("e34-open", "∀xT(x,y)", accepted=True, category="open_formula", explanation="y serbesttir; nihai çözüm olamaz."),
        _syntax_fixture("e34-direction", "∀x∃yT(y,x)", accepted=True, category="sentence", explanation="Herkes birisi tarafından tanınır."),
        _syntax_fixture("e34-at-most-one", "∀x∀y((K(x) ∧ K(y)) → x=y)", accepted=True, category="sentence", explanation="En çok bir koordinatör vardır."),
        _syntax_fixture("e34-unclosed", "∀x(F(x) → G(x)", accepted=False, issue_code="parenthesis.unclosed", explanation="Koşul parantezi kapanmamıştır."),
        _syntax_fixture("e34-arity", "T(a)", accepted=False, issue_code="predicate.arity_mismatch", explanation="T iki rol ister."),
        _syntax_fixture("e34-unknown", "P(a)", accepted=False, issue_code="predicate.unknown", explanation="P anahtarda tanımlı değildir."),
        _syntax_fixture("e34-quantifier-name", "∃aF(a)", accepted=False, issue_code="quantifier.variable_expected", explanation="Niceleyici ad değil değişken bağlar."),
        _syntax_fixture("e34-multiple-connective", "F(a) ∧ G(a) → H(a)", accepted=False, issue_code="connective.multiple_unparenthesized", explanation="Kapsam parantezsiz bırakılmıştır."),
    ]
    lesson["workshop_cases"] = [
        {
            "id": "e34-case-mentor",
            "source": "Her mentor bir katılımcıyı tanıyor.",
            "domain": "Atölyedeki insanlar.",
            "key": ["F(x): x mentor", "G(x): x katılımcı", "T(x,y): x, y'yi tanıyor"],
            "assumptions": ["Mentor veya katılımcı bulunduğu ayrıca varsayılmıyor."],
            "controlled_paraphrase": "Her x için, x mentorsa x'in tanıdığı katılımcı bir y vardır.",
            "readings": [{"context_condition": "Tanık her mentora göre değişebilir.", "formula": "∀x(F(x) → ∃y(G(y) ∧ T(x,y)))", "back_translation": "Her mentorun tanıdığı en az bir katılımcı vardır."}],
            "loss_report": ["Tanımanın zamanı ve derecesi temsil edilmiyor."],
            "peer_review_prompt": "Tanıyan ve tanınan rollerini, ardından y'nin bağlanmasını denetle.",
        },
        {
            "id": "e34-case-ambiguous-mentor",
            "source": "Her katılımcı bir mentoru tanıyor.",
            "domain": "Atölyedeki insanlar.",
            "key": ["F(x): x mentor", "G(x): x katılımcı", "T(x,y): x, y'yi tanıyor"],
            "assumptions": ["'Bir mentor' ifadesinin kapsamı bağlama göre değişebilir."],
            "controlled_paraphrase": "Katılımcıların mentor tanıkları ayrı olabilir veya konuşmada tek ortak mentor öne çıkarılmış olabilir.",
            "readings": [
                {"context_condition": "Her katılımcının kendi mentoru olabilir.", "formula": "∀x(G(x) → ∃y(F(y) ∧ T(x,y)))", "back_translation": "Her katılımcı için tanıdığı en az bir mentor vardır."},
                {"context_condition": "Belli bir ortak mentor kastediliyor.", "formula": "∃y(F(y) ∧ ∀x(G(x) → T(x,y)))", "back_translation": "Bütün katılımcıların tanıdığı bir mentor vardır."},
            ],
            "loss_report": ["'Bir' sözcüğünün söylemde yeni mi bilinen mi olduğu temsil edilmiyor."],
            "peer_review_prompt": "Önce ortak/bağımlı tanık kararını ve niceleyici sırasını denetle.",
        },
        {
            "id": "e34-case-ambiguous-negation",
            "source": "Herkes geç kalmadı.",
            "domain": "Atölyedeki insanlar.",
            "key": ["H(x): x geç kaldı"],
            "assumptions": ["Türkçe yüzey yapı olumsuzluğun kapsamını tek başına sabitlemiyor."],
            "controlled_paraphrase": "En az bir kişi geç kalmadı veya hiç kimse geç kalmadı.",
            "readings": [
                {"context_condition": "Bazı kişilerin zamanında geldiği vurgulanıyor.", "formula": "¬∀xH(x)", "back_translation": "Herkesin geç kaldığı doğru değildir."},
                {"context_condition": "Toplantıya herkes zamanında geldi.", "formula": "∀x¬H(x)", "back_translation": "Hiç kimse geç kalmadı."},
            ],
            "loss_report": ["Konuşmacının düzeltme veya şaşırma vurgusu temsil edilmiyor."],
            "peer_review_prompt": "Hepsi değil ile hiçbiri güç farkını denetle.",
        },
    ]
    lesson["symbolization_fixtures"] = [
        _symbolization_fixture("e34-every-mentor-participant", "Her mentor bir katılımcıyı tanıyor.", [("∀x(F(x) → ∃y(G(y) ∧ T(x,y)))", "Tanık mentora bağlıdır.", "Her mentorun tanıdığı bir katılımcı vardır.")], [("∀x(F(x) → ∃y(G(y) ∧ T(x,y)))", True, None, "Bağımlı tanık doğrudur."), ("∃y(G(y) ∧ ∀x(F(x) → T(x,y)))", False, "translation.quantifier_order", "Ortak tanık gereksizce dayatılmıştır."), ("∀x(F(x) → ∃y(G(y) ∧ T(y,x)))", False, "translation.argument_order", "Bağıntı yönü ters çevrilmiştir.")], teaching_point="Bağımlılık ve rol sırası ayrı denetlenir."),
        _symbolization_fixture("e34-shared-participant", "Bütün mentorların tanıdığı bir katılımcı vardır.", [("∃y(G(y) ∧ ∀x(F(x) → T(x,y)))", "Katılımcı ortak tanıktır.", "Bir katılımcı vardır ve bütün mentorlar onu tanır.")], [("∃y(G(y) ∧ ∀x(F(x) → T(x,y)))", True, None, "Ortak tanık dışta seçilmiştir."), ("∀x(F(x) → ∃y(G(y) ∧ T(x,y)))", False, "translation.quantifier_order", "Ayrı tanıklar ortak kişiyi garanti etmez.")], teaching_point="Ortak tanık ∀ katmanından önce seçilir."),
        _symbolization_fixture("e34-every-participant-mentor-ambiguous", "Her katılımcı bir mentoru tanıyor.", [("∀x(G(x) → ∃y(F(y) ∧ T(x,y)))", "Mentor her katılımcıya göre değişebilir.", "Her katılımcının tanıdığı bir mentor vardır."), ("∃y(F(y) ∧ ∀x(G(x) → T(x,y)))", "Bağlam tek ortak mentor seçer.", "Bütün katılımcıların tanıdığı bir mentor vardır.")], [("∀x(G(x) → ∃y(F(y) ∧ T(x,y)))", True, None, "Bağımlı tanık okumasıdır."), ("∃y(F(y) ∧ ∀x(G(x) → T(x,y)))", True, None, "Ortak tanık okumasıdır."), ("∀x(G(x) → ∃y(F(y) ∧ T(y,x)))", False, "translation.argument_order", "Katılımcı tanınan role geçirilmiştir.")], teaching_point="İki kabul okuması ayrı bağlam koşulu taşır."),
        _symbolization_fixture("e34-everyone-not-late", "Herkes geç kalmadı.", [("¬∀xH(x)", "Hepsi değil okuması.", "En az bir kişi geç kalmadı."), ("∀x¬H(x)", "Hiçbiri okuması.", "Hiç kimse geç kalmadı.")], [("¬∀xH(x)", True, None, "Hepsi değil okumasıdır."), ("∀x¬H(x)", True, None, "Hiçbiri okumasıdır."), ("¬∃x¬H(x)", False, "translation.negation_scope", "Bu herkesin geç kaldığını söyler.")], teaching_point="Bağlamsız yüzey iki savunulabilir dala ayrılır."),
        _symbolization_fixture("e34-only-ada", "Yalnız Ada koordinatördür.", [("(K(a) ∧ ∀x(K(x) → x=a))", "Varlık ve teklik birlikte kurulur.", "Ada koordinatördür ve başka koordinatör yoktur.")], [("(K(a) ∧ ∀x(K(x) → x=a))", True, None, "Yalnızlık eksiksizdir."), ("∀x(K(x) → x=a)", False, "translation.quantifier_scope", "Ada'nın koordinatör olması garanti edilmemiştir."), ("K(a)", False, "translation.identity_missing", "Başka koordinatörler dışlanmamıştır.")], teaching_point="Yalnızlık varlık ile üst sınırı birlikte ister."),
        _symbolization_fixture("e34-knows-other", "Herkes kendisi dışında birini tanır.", [("∀x∃y(x≠y ∧ T(x,y))", "Her x için farklı y seçilir.", "Herkes kendisinden farklı en az birini tanır.")], [("∀x∃y(x≠y ∧ T(x,y))", True, None, "Başka kişi koşulu açıktır."), ("∀x∃yT(x,y)", False, "translation.distinctness_missing", "Kişi kendisini seçebilir."), ("∀x∃y(x≠y ∧ T(y,x))", False, "translation.argument_order", "Yön ters çevrilmiştir.")], teaching_point="'Başka' kimlik yadsıması gerektirir."),
        _symbolization_fixture("e34-everyone-known", "Herkes birisi tarafından tanınır.", [("∀x∃yT(y,x)", "Tanıyan y, tanınan x'e bağlı seçilir.", "Her kişi için onu tanıyan biri vardır.")], [("∀x∃yT(y,x)", True, None, "Roller doğrudur."), ("∀x∃yT(x,y)", False, "translation.argument_order", "Tanıyan ve tanınan ters çevrilmiştir."), ("∃y∀xT(y,x)", False, "translation.quantifier_order", "Tek ortak tanıyan gereksizce eklenmiştir.")], teaching_point="Geri çeviri bağıntı yönünü sınar."),
        _symbolization_fixture("e34-no-mentor-late", "Hiçbir mentor geç kalmadı.", [("∀x(F(x) → ¬H(x))", "Her mentor için geç kalma yadsınır.", "Hiçbir mentor geç kalmadı.")], [("∀x(F(x) → ¬H(x))", True, None, "Kısıtlı hiçbiri doğrudur."), ("¬∀x(F(x) → H(x))", False, "translation.negation_scope", "Bu yalnız bütün mentorların geç kaldığını reddeder."), ("∀x(¬F(x) → H(x))", False, "translation.negation_scope", "Olumsuzluk ana yüklemden kısıtlayıcı sınıfa taşınmıştır.")], teaching_point="Hepsi değil ile hiçbiri ayrılır."),
    ]
    return lesson
